...
from .models import (Course, UserCourseRelation, Lesson,
                     UserLessonRelation, LessonComment, Review)
...
def course_review_list(request: HttpRequest, course_id):
    ...
    user_course_list = UserCourseRelation.objects.filter(
        course=course, review__isnull=False)
    print(f'User course list: {user_course_list}')
    return render(request,
                  'courses/course_review_list.html',
                  {'user_courses': user_course_list, 'course': course})
...
def set_course_review(request: HttpRequest, course_id):
    ...
    review, _ = Review.objects.get_or_create(user_course_relation=user_course)
    ...
    if not courses:
        if search_query:
            ...
            courses = Course.objects.filter(active=True).prefetch_related(
                Prefetch('owner', queryset=User.objects.all().only('username')))
